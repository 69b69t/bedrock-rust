...
//copied from https://github.com/coderbot16/java-rand/blob/master/src/lib.rs
//the only thing i changed was making the state public
...
impl Random {
    ...
    fn next_gaussian_pair(&mut self) -> (f64, f64) {
        let mut next_candidate = || {
            let v = (2.0 * self.next_f64() - 1.0, 2.0 * self.next_f64() - 1.0);
